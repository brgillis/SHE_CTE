...
def vis_and_mer_listfiles(workdir):
    ...
    mer_products = []
    for batch_coords in np.array_split(sky_coords, NUM_EXPOSURES):
    ...
    return vis_listfile, mer_listfile
...
def hdf5_listfile(workdir, vis_and_mer_listfiles):
    ...
    vis_listfile, _ = vis_and_mer_listfiles
...
def test_executable_vis_only(workdir, vis_and_mer_listfiles):
    ...
    vis_listfile, mer_listfile = vis_and_mer_listfiles
    ...
    print(output_vis_batches)
    assert len(output_vis_batches) == NUM_EXPOSURES

    for i, vis_batch in enumerate(output_vis_batches):
...
def test_executable_vis_and_hdf5(workdir, vis_and_mer_listfiles, hdf5_listfile):
    ...
    for i, (vis_batch, hdf5_batch) in enumerate(zip(output_vis_batches, output_hdf5_batches)):
        # First batch should contain all exposures, second should contain 2 onwards, third 3 onwards etc...
        assert vis_batch == vis_prods[i:]
