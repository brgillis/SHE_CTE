...
#    config_data = load_bfd_configuration(mode='training')
    ...
    # find a valid detector
    valid_detector = None
    for exposure in data_stack.exposures:
        for det in exposure.detectors.ravel():
            if det is None:
                continue
            valid_detector = det
    ...
    for row in data_stack.detections_catalogue:
        ...
        gal_id = row[simc_tf.ID]
        gal_x_world = row[simc_tf.gal_x_world]
        gal_y_world = row[simc_tf.gal_y_world]
        ...
        if config_data['isTarget'] == True:
            if bfd_info.lost:
                ...
                bfd_moments_table.add_row({bfdm_tf.ID: gal_id,
                                           ...
                                           bfdm_tf.x_world: bfd_info.x,
                                           bfdm_tf.y_world: bfd_info.y,
...
def bfd_perform_integration(target_file=None, template_file=None):
    ...
    nfactor2 = 10.
    nthreads = 1
    call = ["boostTest", "--targetFile", target_file, "--templateFile", template_file, "--selectSN", str(sn1) + "," + str(
