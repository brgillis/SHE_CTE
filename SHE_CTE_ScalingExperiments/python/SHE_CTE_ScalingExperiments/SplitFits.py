""" @file SplitFits.py
    ...
    Executable for splitting a single exposure into smaller files, one per CCD.
"""
...
from .split_fits import split_fits_from_args
...
def defineSpecificProgramOptions():
    ...
    parser.add_argument('--input_fits_json', type=str,
                        help="json containing a dict of the fits files for the given exposure", required=True)

    

    # Output data
    parser.add_argument('--output_json', type=str,
                        help='Desired name of the output json containing a dict of the newly created files', required=True)

    parser.add_argument('--timing_info', type=str,
                        help='Timing information for the task', required=True)
...
def mainMethod(args):
    ...
    logger.debug('#')
    logger.debug('# Entering SHE_CTE_SplitFits mainMethod()')
    logger.debug('#')

    exec_cmd = get_arguments_string(args, cmd="E-Run SHE_CTE " + SHE_CTE.__version__ + " SHE_CTE_SplitFits",
                                    )#store_true=["profile", "debug", "webdav_archive"])
    logger.info('Execution command for this step:')
    logger.info(exec_cmd)

    split_fits_from_args(args)
